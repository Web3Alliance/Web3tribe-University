...
/**
 * Shared by /student/wallet and /instructor/wallet. Wallet balance and
 * transaction history were previously student-only — but instructors earn
 * real W3TR too (course_publish_bonus on approval, instructor_earning from
 * cohort activity, high_rating_bonus, instructor_milestone) with nowhere to
 * actually see the balance or how they earned it. The underlying data was
 * never role-restricted; only the route was missing.
 */
export async function WalletView({ profileId }: { profileId: string }) {
  const supabase = await createClient();

  const [{ data: wallet }, { data: transactions }] = await Promise.all([
    supabase.from("w3tr_wallets").select("*").eq("profile_id", profileId).single(),
    supabase
