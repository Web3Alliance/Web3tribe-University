...
function MessagesContent() {
  ...
  const fetchFollowing = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('user_follows')
        .select(`following_id, following:users!user_follows_following_id_fkey(id, full_name, email, profile_picture_url)`)
        .eq('follower_id', user.id)
      if (error) throw error
      setFollowingUsers(data?.map(f => f.following) || [])
    } catch (error) {
      setFollowingUsers([])
    }
  }
  ...
  const sendMessage = async () => {
    ...
    if (!profile || profile.w3tr_balance < 1) {
      toast({ title: "Insufficient Balance", description: "You need 1 W3TR to send a message", variant: "destructive" })
      return
    }
    ...
    try {
      ...
      await sendNotification(
        ...
        user.id,
        'message',
        `${profile?.full_name} sent you a message`
      )

      setNewMessage("")
      await loadConversation(selectedUserId)
      fetchConversations()
      toast({ title: "Message Sent", description: "1 W3TR has been deducted from your balance" })
    } catch (error: any) {
      ...
      <div className="min-h-screen bg-background pb-20">
        <div className="max-w-lg mx-auto px-4 py-6 space-y-4">
                ...
                <div className="space-y-4">
                  ...
                  {searchResults.length > 0 ? (
                    <div className="space-y-2">
                      ...
                      {searchResults.map(u => (
                        <Card key={u.id} className="cursor-pointer hover:bg-accent transition-colors" onClick={() => startConversation(u.id)}>
                    ...
                    <p className="text-sm text-muted-foreground text-center py-4">Search for users to start a conversation</p>
                  )}
                </div>
              </DialogContent>
            </Dialog>
          ...
          {conversations.length === 0 ? (
            <Card><CardContent className="pt-6 text-center text-muted-foreground">No conversations yet</CardContent></Card>
          ) : (
            conversations.map(conv => (
              <Card key={conv.user_id} className="cursor-pointer hover:bg-accent transition-colors" onClick={() => router.push(`/messages?user=${conv.user_id}`)}>
                <CardContent className="pt-6">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground mt-1">{new Date(conv.last_message_time).toLocaleString()}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
    ...
    <div className="min-h-screen bg-background flex flex-col">
      <div className="sticky top-0 z-30 bg-background/95 backdrop-blur border-b">
        <div className="flex items-center gap-3 px-4 h-14 max-w-lg mx-auto">
          <Button variant="ghost" size="icon" className="h-9 w-9" onClick={() => router.push('/messages')}>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 max-w-lg mx-auto w-full space-y-3">
        {messages.map(msg => (
          <div key={msg.id} className={`flex ${msg.sender_id === user.id ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[80%] rounded-lg px-4 py-2 ${msg.sender_id === user.id ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>
      ...
      <div className="sticky bottom-0 bg-background border-t p-4 max-w-lg mx-auto w-full">
        ...
        <p className="text-xs text-muted-foreground mt-2 text-center">1 W3TR will be charged per message</p>
      </div>
    </div>
  )
}
